...
type
  PropKind* = enum
    pkFloat, pkVec2, pkVec4, pkDir, pkBool
...
proc igVec4(color: Color): ImVec4 = igVec4(color.r, color.g, color.b, color.a)
...
proc encodeHook*(a: ImVec2, v: var KdlNode, name: string) = 
  v = initKNode(name, args = @[a.x.initKFloat, a.y.initKFloat])

proc encodeHook*(a: ImVec4, v: var KdlNode, name: string) = 
  v = initKNode(name, args = @[a.x.initKFloat, a.y.initKFloat])

# proc styleToToml*(style: ImStyle, ignoreProps: openArray[string] = defaultIgnoreProps, ignoreColors: openArray[string] = [], colorProc: proc(col: ImVec4): TomlValueRef = toTArray): TomlValueRef = 
...
# proc styleFromToml*(node: TomlValueRef, ignoreProps: openArray[string] = defaultIgnoreProps, ignoreColors: openArray[string] = [], colorProc: proc(col: TomlValueRef): ImVec4 = colorToVec4): ImStyle = 
...
#   for name, field in result.style.fieldPairs:
#     if name == "colors" or name in ignoreProps:
#       continue
...
# proc styleFromToml*(path: string, ignoreProps: openArray[string] = defaultIgnoreProps, ignoreColors: openArray[string] = [], colorProc: proc(col: TomlValueRef): ImVec4 = colorToVec4): ImStyle = 
